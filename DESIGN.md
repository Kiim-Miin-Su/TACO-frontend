...
> 레퍼런스: ① Lantiv Scheduling Studio(정보 구조·스케줄링 UX) ② 인프런(시각 언어·타이포·톤).
> 상태: **컨펌 대기** — 승인 후 이 문서가 프론트 디자인의 단일 기준이 된다.
...
- **Phase A — 기반**: @theme 토큰(§2.1·§3·§7: max-width·타이포·radius) + Pretendard 도입 → 공용 컴포넌트 6종(§5) → 인라인 스타일·arbitrary 텍스트 일괄 치환 → 빌드·e2e 회귀 확인. (토큰 교체로 시각 변화는 있으나 **동작 변화 0**)
- **Phase B — 핵심 페이지**: 대시보드 → 캘린더(Lantiv식 툴바 2행 압축·ⓘ 팝오버) → 강사 페이(모달 전환) → 출석부 → 학생·부모(목록 우선) → 승인 센터 (§2 배정표 + §8 흐름 적용, 페이지마다 스크린샷 QA)
- **Phase C — 잔여 페이지 + 회귀**: 상담·결제·지출·보고서·관리자 배정표 적용, 1280/1024px 오버플로 스캔, 문서 마감
