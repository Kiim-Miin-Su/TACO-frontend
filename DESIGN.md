...
### R3. TN Academy 공식 브랜드 자산 (TBO-49, 2026-07-22)

- 기업 정체성 마크는 공식 사이트의 **브론즈 원형 궤도 + TN 모노그램 + 차콜**을 따른다. 업무 상태/액션의
  Primer 색상과 기업 로고 색상은 책임을 섞지 않는다.
- 벡터 단일 원본은 `public/brand/tn-mark.svg`, 화면 렌더는 `components/brand/BrandMark.tsx`다. 로그인·가입과
  사이드바가 동일 컴포넌트를 사용하며 외부 CDN에 의존하지 않는다.
- 브라우저 파생 자산은 같은 원본에서 만든 `app/favicon.ico`(32×32), `app/icon.png`(512×512),
  `app/apple-icon.png`(180×180)다. Next.js App Router의 metadata file convention이 `<head>`를 자동 생성한다.
- 브랜드 exact color는 `lib/brand.ts`의 `ACADEMY_BRAND.colors`가 코드 권위다. 아래 §7 “직접 hex 금지”의
  예외는 공식 브랜드 SVG 내부와 이 객체뿐이며, 업무 버튼·상태·차트에는 사용하지 않는다.
...
- **기업 마크 한정**: TN Academy 브론즈/차콜은 §R3 경계에서만 사용한다. 로그인 브랜드 패널은
  `ACADEMY_BRAND.colors`를 읽고, 그 외 업무 UI의 semantic tone은 기존 Primer 토큰을 유지한다.
