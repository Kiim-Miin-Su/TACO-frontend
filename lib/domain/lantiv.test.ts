...
describe('sessionEditPatch — 편집 폼 패치 빌드(모달·우측 패널 공통 규칙)', () => {
  ...
  it('[R-9] 시작 = 종료만 throw · 종료 < 시작은 익일 종료(자정 크로스)로 허용', () => {
    expect(() => sessionEditPatch({ ...d, endTime: '14:00' }, false)).toThrow(); // 같은 시각 = 무효
    const p = sessionEditPatch({ ...d, startTime: '23:00', endTime: '01:00' }, false); // 익일 01:00 종료
    expect(p.startTime).toBe('23:00');
    expect(p.endTime).toBe('01:00'); // BE가 +1440 래핑해 durationMinutes(120)로 저장
  });
});

describe('cloneSessionBody — Ctrl+C/V·Ctrl+드래그 복제(무결성 규칙)', () => {
  const src = row({ courseId: 10, instructorId: 1, roomId: 2, durationMinutes: 90, topic: '주제', memo: 'm', color: '#111111', isPublic: true, seriesId: 7, status: 'held', instructorAttendance: 'present' });

  it('시작=커서 시각, 길이 유지, status=scheduled 고정(진행 이력·출결·시리즈 미승계)', () => {
    const b = cloneSessionBody(src, { date: '2026-07-08', startMin: 14 * 60 + 30 });
    expect(b).toMatchObject({
      courseId: 10, instructorId: 1, roomId: 2, sessionDate: '2026-07-08', isPublic: true,
...
// [R2 2026-07-06] 스플릿 컴팩트 밀도 — 단계 경계 검증(TBO-16 #1)
import { densityOf } from './lantiv';
describe('densityOf — 스플릿 서브열 폭 → 텍스트 밀도', () => {
  it('단일 그리드는 폭 무관 full(스플릿 아님)', () => {
    expect(densityOf(10, false)).toBe('full');
  });
  it('경계값: 80=full · 46~79=title(가로 축약) · 24~45=vtitle(세로 글) · <24=color(색상만)', () => {
    expect(densityOf(80, true)).toBe('full');
    expect(densityOf(79, true)).toBe('title');
    expect(densityOf(46, true)).toBe('title');
    expect(densityOf(45, true)).toBe('vtitle');
    expect(densityOf(24, true)).toBe('vtitle');
