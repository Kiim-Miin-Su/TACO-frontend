...
const emptySlice = {
  ...
  instructors: [{ id: 1, name: '박지훈', englishName: 'Jihoon Park', defaultHourlyRate: 0, canTeachKinder: false }],
...
describe('scheduleRequests — 배지·To-do 동일 모집단(R1)', () => {
  it('관리자: pending 요청 = To-do 카운트 + /admin 배지 (승인센터 리스트와 같은 기준)', () => {
    ...
    const { items, count, badges } = buildTasks(s, 'manager');
    ...
    expect(badges.byDestination['/admin/approvals']).toBe(2); // 관리자 내부 승인 버튼도 같은 props 값
    expect(sumTaskBadges(badges.byNavigation)).toBe(count);
  });
...
// 미해결 업무는 열람만으로 해결되지 않는다. 서버 상태가 바뀌어 모집단에서 빠질 때만 배지가 내려간다.
describe('알림 뱃지 미해결 상태 유지', () => {
  ...
  it('탭을 열람해도 미해결 배지는 유지되고 해결 데이터가 오면 사라진다', () => {
    const s = { ...emptySlice, classSessions: [canceled] };
    expect(navBadges(s, 'manager')['/calendar']).toBe(1);
    expect(navBadges(s, 'manager', undefined, { calendar: '2099-01-01T00:00:00.000Z' })['/calendar']).toBe(1);
    expect(navBadges({ ...emptySlice, classSessions: [] }, 'manager')['/calendar']).toBeUndefined();
  });
});
