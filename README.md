...
- **유저별 추가(2026-07-03)**: 전역 "+ 스케줄 추가"(현행)와 별개로, 스플릿 컬럼 헤더의 **＋ 버튼** 또는 컬럼 빈 공간 더블클릭 = **그 유저 프리필**로 추가 모달(같은 모달 재사용) — 강사 컬럼이면 수업 탭 강사 프리필(변경 가능), 가용/불가 탭은 그 유저가 소유자로 프리필. 학생 가용/불가도 학생 컬럼에서 동일하게 즉시 생성·수정.
