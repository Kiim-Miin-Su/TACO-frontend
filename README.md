...
- **스플릿 컬럼별 가용/불가 밴드(2026-07-03)**: 스플릿 뷰에서는 각 컬럼에 **그 컬럼 유저(강사/학생/강의실)의 가용(초록)/불가(회색 사선)**가 항상 표시됩니다 — 여러 강사의 빈 시간을 나란히 비교하며 배치 가능. 표시 전용이며 편집(드래그·삭제)은 개인 필터 중 선택 유저의 밴드만. 날짜·소유자 매칭 규칙은 `blocksOnDate`(lib/domain/schedule) 단일 소스.
