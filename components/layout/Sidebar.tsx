...
type Item = { label: string; icon: React.FC<any>; active?: boolean; badge?: string };

const groups: { title: string; items: Item[] }[] = [
  {
    title: '운영',
    items: [
      { label: '대시보드', icon: IconHome, active: true },
      { label: '학생 · 부모', icon: IconUsers },
      { label: '수강 등록', icon: IconBook },
    ],
  },
  {
    title: '입금',
    items: [{ label: '결제 · 수납', icon: IconCard, badge: '3' }],
  },
  {
    title: '출금',
    items: [
      { label: '강사 페이', icon: IconWallet },
      { label: '지출 · 비품', icon: IconReceipt },
    ],
  },
  {
    title: '기타',
    items: [
      { label: '수업 보고서', icon: IconReport },
      ...
      <nav className="flex-1 overflow-y-auto py-3">
        {groups.map((g) => (
          <div key={g.title} className="px-3 mb-3">
            <div className="px-2 mb-1 text-[11px] font-semibold uppercase tracking-wide text-fg-subtle">
              {g.title}
            </div>
            {g.items.map((it) => {
              const Icon = it.icon;
              return (
                <a
                  ...
                  className={`flex items-center gap-2.5 px-2 h-8 rounded-md text-[13px] mb-0.5 ${
                    it.active
                      ? 'bg-neutral-subtle font-semibold text-fg'
