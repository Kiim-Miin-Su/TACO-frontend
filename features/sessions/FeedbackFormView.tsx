...
export function FeedbackFormView({ sessionId, studentId }: { sessionId: number; studentId: number }) {
  ...
  if (!session || !student) {
    return (
      <div className="p-6 max-w-[760px] mx-auto text-fg-muted">대상을 찾을 수 없습니다.</div>
    );
  }
  ...
  return (
    <div className="p-6 max-w-[760px] mx-auto space-y-5">
      <div>
        <Link href={`/sessions/${sessionId}`} className="text-[12px] text-fg-muted hover:underline">← 수업 상세</Link>
        ...
        <p className="text-[13px] text-fg-muted mt-0.5">{course?.name} · {session.sessionDate} · {session.topic ?? '주제 미정'}</p>
      </div>
        ...
        <div className="p-4 text-[13px]">
          {parent ? (
            <div className="flex flex-wrap items-center gap-x-4 gap-y-1">
              <span><b>{parent.name}</b> ({link?.relation ?? '보호자'})</span>
      ...
      <SectionCard title="피드백 작성">
        <div className="p-4 space-y-3">
          <div>
            ...
            <textarea className="input h-28 py-2 leading-relaxed" placeholder="오늘 수업 내용·태도·성취" value={content} onChange={(e) => setContent(e.target.value)} />
          </div>
          <div>
            <span className="block text-[12px] font-medium text-fg-muted mb-1">숙제</span>
            <textarea className="input h-20 py-2 leading-relaxed" placeholder="다음 수업 전까지" value={homework} onChange={(e) => setHomework(e.target.value)} />
          </div>
          <div className="flex justify-end gap-2">
            <button className="btn" onClick={() => save(false)}>임시 저장</button>
            <button className="btn btn-primary" disabled={!content.trim()} onClick={() => save(true)}>제출 (발송 대기)</button>
          </div>
        </div>
      </SectionCard>
    </div>
