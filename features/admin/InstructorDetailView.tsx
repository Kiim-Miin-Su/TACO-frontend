...
export function InstructorDetailView({ instructorId }: { instructorId: number }) {
  ...
  const canManage = can('admin.area');
      ...
      {(instructor) => {
        ...
        const save = () => {
          ...
          void sudoAction.run(() => update.mutateAsync({ id: instructor.id, patch: {
            ...
            ...(canManageFinance ? { defaultHourlyRate: Number(edit.defaultHourlyRate) || 0 } : {}),
            canTeachKinder: edit.canTeachKinder,
          } }), {
          ...
          <div className="space-y-5">
            <SectionCard title={`${staffDisplayName(instructor)} 강사`} action={<Badge tone="success">활성</Badge>}>
              <div className="p-4 space-y-4">
                ...
                <InstructorProfileFields value={view} onChange={(next) => setEdit(next)} disabled={!edit} showHourlyRate={canManageFinance} />
                ...
                <p className="text-caption text-fg-subtle">활성 수업·계약·스케줄이 있으면 삭제가 거부됩니다. 모든 변경과 삭제는 감사 이력에 남습니다.</p>
              </div>
              {deleteOpen && <ConfirmModal title="강사 삭제" message="강사 계정과 프로필을 소프트 삭제합니다. 감사 이력은 보존됩니다." confirmLabel="삭제" danger onClose={() => setDeleteOpen(false)} onConfirm={() => {
                setDeleteOpen(false);
                void sudoAction.run(() => remove.mutateAsync(instructor.id), {
