...
export function InstructorsView() {
  ...
  return (
    <SectionCard title={`강사 (${rows.length})`} action={canManage ? <button type="button" className="btn btn-sm btn-primary" onClick={() => setCreateOpen(true)}>+ 강사 등록</button> : undefined}>
            ...
            <tbody>
              {rows.map((instructor) => (
                <ClickableTableRow key={instructor.id} href={internalRoute.adminInstructor(instructor.id)} label={`${instructor.name} 강사 상세`}>
                  ...
                  {canViewFinance && <td className="mono">{instructor.defaultHourlyRate.toLocaleString('ko-KR')}원</td>}
